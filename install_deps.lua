...
if TARGET == "LINUX" then
  ...
  if inst("piper") then
    ...
    mv("libpiper/install/libpiper_phonemize.so.*.*.*", "lib/libpiper.so")
    mv("libpiper/install/libonnxruntime.so.*.*.*", "lib/libonnxruntime.so")
    mv("libpiper/install/libespeak-ng.so.*.*.*.*", "lib/libespeak-ng.so")

...
void Clay_Raylib_Initialize() {
    Clay_SetMeasureTextFunction(Raylib_MeasureText, theme::fonts);
}
    ...
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        switch (renderCommand->commandType)
        {
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                DrawTextEx(fontToUse, temp_render_buffer, (Vector2){boundingBox.x, boundingBox.y}, (float)textData->fontSize, (float)textData->letterSpacing, BLACK);
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
                ...
                DrawTexturePro(
                    imageTexture,
                    {0.f, 0.f, (float)imageTexture.width, (float)imageTexture.height},
